...
async function loadTxns(){const params=new URLSearchParams({limit:TXN_LIMIT,offset:txnOffset});if(window._drillType){params.set('txn_type',window._drillType);params.set('exclude_transfers','true');}const search=$('t-search')?.value,account=$('t-account')?.value;const catVal=$('t-category')?.value,from=$('t-from')?.value,to=$('t-to')?.value;if(search)params.set('search',search);if(account)params.set('account_id',account);if(catVal==='none')params.set('category_id','none');else if(catVal)params.set('category_id',catVal);if(from)params.set('start_date',from);if(to)params.set('end_date',to);const recur=$('t-recurring')?.checked;if(recur)params.set('recurring','true');const data=await api('/api/transactions?'+params);txnTotal=data.total;const txns=data.transactions;_txnList=txns;const pages=Math.max(1,Math.ceil(txnTotal/TXN_LIMIT));const page=Math.floor(txnOffset/TXN_LIMIT)+1;const showBal=data.has_balance;document.querySelectorAll('.bal-col').forEach(el=>{el.style.display=showBal?'':'none';});
  $('txn-summary').textContent=`${txnTotal.toLocaleString()} transactions`;$('t-page-lbl').textContent=`${page} / ${pages}`;$('t-prev').disabled=txnOffset===0;$('t-next').disabled=txnOffset+TXN_LIMIT>=txnTotal;if(typeof updateFilterBadge==='function')updateFilterBadge();if(!txns.length){$('txn-tbody').innerHTML=`<tr><td colspan="6" class="empty">No transactions match</td></tr>`;return;}$('txn-tbody').innerHTML=txns.map(t=>{const tb=t.is_transfer?`<span class="badge-transfer" onclick="toggleTransfer('${t.id}')" title="Click to unmark">↔ transfer</span> `:'';
    const srcBadge=t.source==='manual'?'<span class="badge" style="background:#1e3a5f;color:#7dd3fc;font-size:.62rem;margin-right:.3rem" title="Manually entered">✎ manual</span> ':t.source==='csv'?'<span class="badge" style="background:#3b1f0b;color:#fde68a;font-size:.62rem;margin-right:.3rem" title="CSV import">⬆ csv</span> ':'';return`<tr style="${t.is_transfer?'opacity:0.5':''}" onclick="openTxnModal('${t.id}')"><td class="tc-date" style="white-space:nowrap;color:#64748b">${fmtDate(t.posted)}</td><td class="tc-desc"><div style="font-size:.83rem">${t.recurring?'<span style="color:#c084fc;font-size:.68rem;margin-right:.3rem" title="Recurring">↻</span>':''}${srcBadge}${tb}${esc(t.payee||t.description||'—')}</div>${t.payee&&t.description!==t.payee?`<div style="font-size:.72rem;color:#475569">${esc(t.description)}</div>`:''}</td><td class="tc-acct" style="font-size:.75rem;color:#475569">${esc(t.account_name||'')}</td><td class="tc-cat">${catBadge(t.category_id,t.category)}</td><td class="tc-amt ${t.amount<0?'amt-neg':'amt-pos'}" style="text-align:right">${fmt(t.amount)}</td><td class="tc-bal bal-col ${t.running_balance!=null?(t.running_balance<0?'amt-neg':'amt-pos'):''}" style="text-align:right;display:none">${t.running_balance!=null?fmt(t.running_balance):''}</td><td class="tc-edit"><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openTxnModal('${t.id}')">Edit</button></td></tr>`;}).join('');
  // Append total row
